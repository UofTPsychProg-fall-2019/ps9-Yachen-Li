...
ipip.l <- ipip %>% 
  gather('A_1':'O_10', key = BigFive, value = Score)
...
ipip.l <- ipip.l %>% separate(BigFive,into=c('trait','item'),sep="_")

# Calculate averages for each participant (coded as RID) and trait:
ipip.comp <- ipip.l %>% group_by(RID,trait) %>% mutate(traitMean=mean(Score))
...
exer.avg <- ipip.comp %>% 
  ...
  summarise(
    avg=mean(traitMean),
    sem=sd(traitMean)/sqrt(n_distinct(RID)))
...
age.avg <- ipip.comp %>%
  group_by(trait)%>%
  mutate(corrcoef = cor(age,traitMean))
